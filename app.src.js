...
const UNISAT_API      = 'https://open-api.unisat.io';  // used for read-only public endpoints only
const MARKET_API      = 'https://btcnative-market.galanin.workers.dev';  // all authenticated calls proxied here
...
// UniSat market API — all calls proxied through the market worker (server-side key, CORS-safe)
// path mapping: /v3/market/domain/auction/list       → /api/proxy/auction-list
//               /v3/market/domain/auction/domain_types → /api/proxy/domain-types (GET)
//               /v3/market/domain/auction/actions     → /api/proxy/auction-actions
async function unisatPost(path, body = {}) {
  try {
    let proxyUrl;
    let method = 'POST';

    if (path.includes('domain_types')) {
      // domain_types is a GET proxy
      proxyUrl = `${MARKET_API}/api/proxy/domain-types`;
      method = 'GET';
    } else if (path.includes('auction/list')) {
      proxyUrl = `${MARKET_API}/api/proxy/auction-list`;
    } else if (path.includes('auction/actions')) {
      proxyUrl = `${MARKET_API}/api/proxy/auction-actions`;
    } else {
      // Unknown path — log and bail
      console.warn('unisatPost: unmapped path', path);
      return null;
    }
    ...
    clearTimeout(timer);
    if (!r.ok) { console.warn('Market proxy error', r.status, path); return null; }
    const json = await r.json();
    if (json.code !== 0) { console.warn('Market proxy non-zero code', json.code, json.msg); return null; }
    return json.data;
  } catch(e) { console.warn('Market proxy fetch failed', path, e.message); return null; }
}
...
async function _doFetchListings({ domainType, minLength, maxLength, page, pageSize, sort, cacheKey }) {
  ...
  try {
    ...
    const unisatPromise = true  /* proxy always available */ ? unisatPost('/v3/market/domain/auction/list', {
      filter: {
        nftType: 'domain',
    ...
    if (priceEl) {
      _enqueuePriceCheck(name).then(item => {
        ...
        if (item && item.unitPrice) {
          ...
          priceEl.innerHTML += '<span class="name-card__usd"></span>';
          getBtcUsd().then(rate => {
            const u = priceEl.querySelector('.name-card__usd');
            if (u) u.textContent = ' · ' + formatUsd(item.unitPrice, rate);
          });
          // Add buy button
          const buyBtn = document.createElement('button');
          buyBtn.className = 'name-card__buy-btn';
          buyBtn.dataset.name = name;
          buyBtn.dataset.auction = item.auctionId || '';
          buyBtn.dataset.price = String(item.unitPrice);
          buyBtn.textContent = 'Buy';
          buyBtn.onclick = (ev) => { ev.preventDefault(); ev.stopPropagation(); _openCardBuy(buyBtn); };
          card.appendChild(buyBtn);
        } else {
          priceEl.textContent = 'Not listed';
          priceEl.style.color = 'var(--color-text-faint)';
          priceEl.style.fontSize = 'var(--text-xs)';
        }
      }).catch(() => {
        if (priceEl.isConnected) { priceEl.textContent = 'Not listed'; priceEl.style.color = 'var(--color-text-faint)'; priceEl.style.fontSize = 'var(--text-xs)'; }
      });
    }
  }
...
function computeBadges(data) {
  ...
  // Confusable-character warning only (no famous-name disclaimer)
...
async function initProfilePage() {
  ...
  (async () => {
    try {
      ...
      // Route through proxy — no key in client
      const infoRes = await fetch(
        `${MARKET_API}/api/proxy/domain-types?domainType=${encodeURIComponent('.' + tldRaw)}`,
  ...
  // Trust warning (confusable characters only) on profile
  const trustWarnEl = qs('#profileTrustWarnings');
  if (trustWarnEl) {
    const confWarn = detectConfusable(name);
    if (confWarn) {
      trustWarnEl.innerHTML = `<div class="trust-warning trust-warning--warn">${confWarn}</div>`;
...
// MARKET_API defined at top of file
...
function openOfferModal() {
  if (!_isWalletConnected()) { _promptConnect('make offers'); return; }
  const params = new URLSearchParams(location.search);
  const name = params.get('name') || '';
...
function _isWalletConnected() {
  ...
  return !!(navBtn && navBtn.dataset.addr);
}

function _promptConnect(actionLabel) {
  // Simple inline toast prompting wallet connection
  const existing = document.getElementById('_walletGateToast');
  if (existing) existing.remove();
  const toast = document.createElement('div');
  toast.id = '_walletGateToast';
  toast.style.cssText = 'position:fixed;bottom:24px;left:50%;transform:translateX(-50%);background:var(--color-surface);border:1px solid var(--color-border);border-radius:var(--radius-lg);padding:12px 20px;font-size:var(--text-sm);color:var(--color-text);z-index:9999;display:flex;align-items:center;gap:12px;box-shadow:0 8px 32px rgba(0,0,0,0.4);';
  toast.innerHTML = `<span style="color:var(--color-text-muted);">Connect a wallet to ${actionLabel}</span><button onclick="initNavWallet();document.getElementById('_walletGateToast')?.remove();" style="background:var(--color-primary);color:#000;border:none;border-radius:var(--radius-sm);padding:6px 14px;font-size:var(--text-xs);font-weight:700;cursor:pointer;white-space:nowrap;">Connect</button><button onclick="this.closest('#_walletGateToast').remove()" style="background:none;border:none;color:var(--color-text-faint);cursor:pointer;font-size:16px;line-height:1;">&times;</button>`;
...
function toggleWatch() {
  ...
  if (!name || !btn) return;
  if (!_isWalletConnected()) { _promptConnect('watch names'); return; }
...
async function fetchAndRenderComps(name) {
  ...
  const data = true  /* proxy always available */ ? await fetchListings({
    domainType: tldRaw,
    minLength: len,
    maxLength: len,
    page: 0,
...
async function delistName(name, auctionId) {
  ...
  try {
    ...
    // Step 1: create_put_off (proxied through worker — no key in client)
    const res = await fetch(`${MARKET_API}/api/proxy/delist-create`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    // Step 3: confirm_put_off (proxied through worker)
    const confirmRes = await fetch(`${MARKET_API}/api/proxy/delist-confirm`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
